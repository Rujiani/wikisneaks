...
describe('User CRUD', () => {
  ...
  describe('POST /api/auth/register', () => {
    ...
    it('does not expose the password hash', async () => {
      const payload = await registerUser()
      const user = await findUserByEmail(payload.email)
      const stored = await db.user.findUnique({ where: { email: payload.email } })
    ...
    it('returns 409 when email is already registered', async () => {
      ...
      expect(res.body).toEqual({ message: 'Email or login is already registered' })
    })
  ...
  describe('GET /api/users', () => {
    ...
    it('returns created users newest first', async () => {
      ...
      expectPublicUser(res.body[0], { login: second.login, email: second.email })
    ...
    it('paginates with limit and offset', async () => {
      ...
      const page = await request(app).get('/api/users').query({ limit: 2, offset: 0 })
      ...
      const next = await request(app).get('/api/users').query({ limit: 2, offset: 2 })
    ...
    it('returns 400 for invalid pagination query', async () => {
      const res = await request(app).get('/api/users').query({ limit: 0, offset: -1 })
  ...
  describe('GET /api/users/:userId', () => {
    it('returns a single user', async () => {
      ...
      expect(res.status).toBe(200)
      expectPublicUser(res.body.user, { login: payload.login, email: payload.email })
  ...
  describe('PATCH /api/users/:userId', () => {
    ...
    it('returns 404 when the user does not exist', async () => {
      const res = await request(app).patch('/api/users/1').send({ extraInfo: 'bio' })
    ...
    it('returns 400 when extraInfo is missing or too long', async () => {
      ...
      const missing = await request(app).patch(`/api/users/${created.id}`).send({})
